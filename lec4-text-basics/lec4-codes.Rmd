...
To see how frequent a word appears, just take som the column. For instance, to get how many times the word `school` appears:
...
Let's try sentiment analysis with dictionary method. First, downlaod [Hu and Liu's opinion lexicon's here](http://www.cs.uic.edu/~liub/FBS/opinion-lexicon-English.rar). The link is also mentioned in class tutorial.
...
In this dictionry, there are two keys ("positive" and "negative"), each maps to a list as its values.
...
(also remember to do other preprocessing steps, or using the object that's after preprocessing.
