...
```{r}
gadarian_3_ <- stm(documents = out$documents,
                 vocab = out$vocab,
                 data = out$meta,
                 prevalence = ~ treated *pid_rep ,
                 K = 3, verbose = FALSE)
plot(gadarian_3)

## explicity estimate effect of covariates on topic proportions
prep_3 <- estimateEffect(1:3 ~ treated * pid_rep, gadarian_3,
                        meta = out$meta)
summary(prep_3)
```
...
```{r}
## oly 
prep_topic1only <- estimateEffect(c(2) ~ treated * pid_rep, gadarian_3,
                        meta = out$meta)
