...
- proprocessing corpus (stemming, removing punctuation, url, stopwords, and use up to 3-grams)
...
We end up with a dataset with size 40000 * 11686.


## try different algorithms

A cautionary note: converting doc.term to matrix can be painsome; it makes your memory 10 times larger. This is because `data.frame` does not take into consideration that the data is sparse.
...
Read the help file and you will find that this packages use $\alpha$ to control the relative strength of LASSO and RIdge. Setting $\alpha$ = 1, we obtain LASSO estimator.
...
## train/validation split to select parameters

How do we know which $\lambda$ to take? We further split our training data to 
...
Instead, we do a grid-search.
We generate a list of lambda, check the prediction performance on each of the  
```{r}
...
I did not use it earlier, because it does not allow you to choose based on F1 score.
...
The performance is significantly worse than the F1 score based on LASSO.

## tree and forest

Now let us move to tree method. A common choice in R is to use `rpart` package, but unfortunately it does not take the type of sparse matrix. We have to limit our data alittle bit to make it work (to demonstrate how it works).

There are many other packages that implement decision trees, and I am not going to show that all. 

Decision tree algorithm is actually very slow; we limit the observation number to 1000 just to show you how it works
...
We can predict with the tree, but since the training data only cantains 1000 samples, the performance is likely to be very bad.
